...
BOOL FindingZipcodeForm_OnFindButtonClicked(HWND hWnd, WPARAM wParam, LPARAM lParam) {
	...
	if (HIWORD(wParam) == BN_CLICKED) {
		...
		SendMessage(GetDlgItem(hWnd, IDC_EDIT_DONG), WM_GETTEXT, (WPARAM)DONG, (LPARAM)inputDong);

		//1.2. 우편번호부에서 입력받은 동을 찾는다.
		postalBook = (PostalBook *)GetWindowLong(hWnd, GWL_USERDATA);
		PostalBookFind(postalBook, inputDong, &indexes, &count);
		...
		while (i < count) {
			...
			item.iSubItem = 1;
			sprintf(totalAddress, "%s %s %s %s %s", postalBook->addresses[indexes[i]].sido,
			...
			if (strcmp(postalBook->addresses[indexes[i]].stBunji, "") != 0) {
				sprintf(totalAddress, "%s %s~%s", totalAddress, 
			...
			item.iSubItem = 2;
			item.pszText = postalBook->addresses[indexes[i]].zipcode;
			SendMessage(GetDlgItem(hWnd, IDC_LIST_ADDRESSES), LVM_SETITEMTEXT, (WPARAM)i, (LPARAM)&item);
